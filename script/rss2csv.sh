...
while IFS="$csv_delimiter" read -r title description pubdate url; do
  echo "Title: \"$title\""
  ...
  echo "$title$csv_delimiter$description$csv_delimiter$pubdate$csv_delimiter$decoded_url$csv_delimiter$sortable_date" >> "$tmp_items"
done < "$tmp_raw_items"
